...
class DianDocument(models.Model):
    ...
    def _create_xml_invoice(self, template_data):
        is_export_invoice = self.document_id.fe_type == '02'
        
        if not is_export_invoice:
        ...
        # QR Code
        if is_export_invoice:
            qrcode_text = f"https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey={template_data['UUID']}"
        else:
        ...
        if self.document_id.currency_id.name != 'COP':
            ...
            # Solo incluir Interoperabilidad para facturas no de exportación
            if not is_export_invoice:
            ...
            exchange_rate = self.document_id.currency_id.inverse_rate
                
            ET.SubElement(totales_cop, 'FctConvCop').text = '%0.9f' % float(exchange_rate)
            ET.SubElement(totales_cop, 'MonedaCop').text = self.document_id.currency_id.name
            ET.SubElement(totales_cop, 'SubTotalCop').text = f"{float(template_data['TotalLineExtensionAmount']) / exchange_rate:.2f}"
            
            # Campos adicionales para exportación
            if is_export_invoice:
                ET.SubElement(totales_cop, 'DescuentoDetalleCop').text = '0.00'
                ET.SubElement(totales_cop, 'RecargoDetalleCop').text = '0.00'
                
            ET.SubElement(totales_cop, 'TotalBrutoFacturaCop').text = f"{float(template_data['TotalLineExtensionAmount']) / exchange_rate:.2f}"
            ET.SubElement(totales_cop, 'TotIvaCop').text = f"{template_data.get('tot_iva_cop', 0) * exchange_rate:.2f}"
            
            # Campos adicionales para exportación
            if is_export_invoice:
                ET.SubElement(totales_cop, 'TotIncCop').text = '0.00'
                ET.SubElement(totales_cop, 'TotBolCop').text = '0.00'
                ET.SubElement(totales_cop, 'ImpOtroCop').text = '0.00'
                ET.SubElement(totales_cop, 'MntImpCop').text = '0.00'
                
            ET.SubElement(totales_cop, 'TotalNetoFacturaCop').text = f"{float(template_data['TotalTaxExclusiveAmount']) / exchange_rate:.2f}"
            
            if is_export_invoice:
                ET.SubElement(totales_cop, 'MntDctoCop').text = '0.00'
                ET.SubElement(totales_cop, 'MntRcgoCop').text = '0.00'
                
            ET.SubElement(totales_cop, 'VlrPagarCop').text = f"{float(template_data['PayableAmount']) / exchange_rate:.2f}"
            ET.SubElement(totales_cop, 'ReteFueCop').text = f"{template_data.get('rete_fue_cop', 0) /exchange_rate:.2f}"
            ET.SubElement(totales_cop, 'ReteIvaCop').text = f"{template_data.get('rete_iva_cop', 0) / exchange_rate:.2f}"
            
            if is_export_invoice:
                ET.SubElement(totales_cop, 'ReteIcaCop').text = '0.00'
                ET.SubElement(totales_cop, 'TotAnticiposCop').text = '0.00'
        ...
        if is_export_invoice and 'DueDate' in template_data:
            ET.SubElement(root, 'cbc:DueDate').text = str(template_data['DueDate'])
        ...
        if is_export_invoice:
            ET.SubElement(cac_address, 'cbc:PostalZone').text = template_data.get('SupplierPostalZone', '000000')
        ...
        if is_export_invoice:
            company_id_attribs['schemeVersionID'] = '1'
            
        ET.SubElement(cac_party_tax_scheme, 'cbc:CompanyID', attrib=company_id_attribs).text = str(template_data['ProviderID'])
        ...
        if is_export_invoice:
            ET.SubElement(cac_registration_address, 'cbc:PostalZone').text = template_data.get('SupplierPostalZone', '000000')
        ...
        ET.SubElement(cac_party_legal_entity, 'cbc:CompanyID', attrib=company_id_attribs).text = str(template_data['ProviderID'])
        ...
        # Agregar Name para exportación
        if is_export_invoice and 'CorporateRegistrationName' in template_data:
            ET.SubElement(cac_corporate_registration_scheme, 'cbc:Name').text = str(template_data['CorporateRegistrationName'])

        cac_contact = ET.SubElement(cac_party, 'cac:Contact')
        if is_export_invoice:
            ET.SubElement(cac_contact, 'cbc:Name').text = str(template_data['SupplierPartyName'])
            ET.SubElement(cac_contact, 'cbc:Telephone').text = str(template_data.get('SupplierTelephone', ''))
        ...
        if is_export_invoice:
            ET.SubElement(cac_address, 'cbc:PostalZone').text = template_data.get('CustomerPostalZone', '000000')
        ...
        if is_export_invoice:
            customer_company_attribs['schemeVersionID'] = '1'
            
        ET.SubElement(cac_party_tax_scheme, 'cbc:CompanyID', attrib=customer_company_attribs).text = str(template_data['CustomerID'])
        ...
        if is_export_invoice:
            ET.SubElement(cac_registration_address, 'cbc:PostalZone').text = template_data.get('CustomerPostalZone', '000000')
        ...
        ET.SubElement(cac_party_legal_entity, 'cbc:CompanyID', attrib=customer_company_attribs).text = str(template_data['CustomerID'])
        
        if is_export_invoice:
            cac_corp_reg_scheme = ET.SubElement(cac_party_legal_entity, 'cac:CorporateRegistrationScheme')
            ET.SubElement(cac_corp_reg_scheme, 'cbc:Name').text = template_data.get('CustomerCorporateRegistrationName', 'No tiene')
        ...
        if is_export_invoice:
            ET.SubElement(cac_payment_means, 'cbc:PaymentID').text = '0'
        else:
            ET.SubElement(cac_payment_means, 'cbc:PaymentID').text = '1234'
            
        if is_export_invoice:
            cac_payment_terms = ET.SubElement(root, 'cac:PaymentTerms')
            ET.SubElement(cac_payment_terms, 'cbc:ReferenceEventCode').text = '2'
            cac_settlement_period = ET.SubElement(cac_payment_terms, 'cac:SettlementPeriod')
            ET.SubElement(cac_settlement_period, 'cbc:DurationMeasure', attrib={'unitCode': 'DAY'}).text = '30'
        ...
        if self.document_id.currency_id.name != 'COP' or is_export_invoice:
            cac_payment_exchange_rate = ET.SubElement(root, 'cac:PaymentExchangeRate')
            exchange_rate = self.document_id.currency_id.inverse_rate
            ET.SubElement(cac_payment_exchange_rate, 'cbc:SourceCurrencyCode').text = 'COP'
            ET.SubElement(cac_payment_exchange_rate, 'cbc:SourceCurrencyBaseRate').text =  '%0.9f' % float(exchange_rate)
            ET.SubElement(cac_payment_exchange_rate, 'cbc:TargetCurrencyCode').text = str(self.document_id.currency_id.name)
            ET.SubElement(cac_payment_exchange_rate, 'cbc:TargetCurrencyBaseRate').text = '1.00'
            ET.SubElement(cac_payment_exchange_rate, 'cbc:CalculationRate').text = '%0.6f' % (1 / float(exchange_rate)) 
        ...
        ET.SubElement(cac_legal_monetary_total, 'cbc:AllowanceTotalAmount', attrib={'currencyID': str(template_data['CurrencyID'])}).text = '{:.2f}'.format(template_data.get('rounding_discount', 0) + self.document_id.invoice_discount)
        ET.SubElement(cac_legal_monetary_total, 'cbc:ChargeTotalAmount', attrib={'currencyID': str(template_data['CurrencyID'])}).text = '{:.2f}'.format(template_data.get('rounding_charge', 0))
        ...
        if is_export_invoice:
            ET.SubElement(cac_legal_monetary_total, 'cbc:PayableRoundingAmount', attrib={'currencyID': str(template_data['CurrencyID'])}).text = '0.00'
    ...
    def _generate_data_invoice_document_xml(self, dc, dcd, data_taxs_xml,  data_lines_xml,  CUFE):
        ...
        if 'InvoicePeriodStartDate' in dcd:
            data.update({'InvoicePeriodStartDate': dcd['InvoicePeriodStartDate']})

        if 'InvoicePeriodEndDate' in dcd:
            data.update({'InvoicePeriodEndDate': dcd['InvoicePeriodEndDate']})
    ...
    # =====================================================
    # METODOS SECTOR SALUD
    # =====================================================

    def _is_health_sector_invoice(self):
        """Determina si la factura pertenece al sector salud"""
        health_operations = ['SS-CUFE', 'SS-CUDE', 'SS-POS', 'SS-SNum', 'SS-Recaudo', 'SS-Reporte', 'SS-SinAporte']
        fe_operation_type = getattr(self.document_id, 'fe_operation_type', None)
        is_health = getattr(self.document_id, 'is_health_sector', False)
        return is_health or (fe_operation_type and fe_operation_type in health_operations)

    def _add_health_sector_extension(self, ext_ubl_extensions, template_data):
        """Agrega la extension XML del sector salud para facturas FEV"""
        move = self.document_id

        # No agregar extension para recaudos (SS-Recaudo)
        fe_operation_type = getattr(move, 'fe_operation_type', '')
        if fe_operation_type == 'SS-Recaudo':
            return

        # Crear extension
        ext_ubl_extension = ET.SubElement(ext_ubl_extensions, 'ext:UBLExtension')
        ext_content = ET.SubElement(ext_ubl_extension, 'ext:ExtensionContent')
        custom_tag = ET.SubElement(ext_content, 'CustomTagGeneral')

        # Interoperabilidad
        interop = ET.SubElement(custom_tag, 'Interoperabilidad')
        group = ET.SubElement(interop, 'Group')
        group.set('schemeName', 'Sector Salud')

        collection = ET.SubElement(group, 'Collection')
        collection.set('schemeName', 'Usuario')

        # Agregar campos del sector salud
        self._add_health_field(collection, 'CODIGO_PRESTADOR', self._get_health_provider_code())
        self._add_health_field(collection, 'MODALIDAD_PAGO', self._get_health_payment_mode())
        self._add_health_field(collection, 'COBERTURA_PLAN_BENEFICIOS', self._get_health_coverage_plan())
        self._add_health_field(collection, 'NUMERO_CONTRATO', self._get_health_contract_number())
        self._add_health_field(collection, 'NUMERO_POLIZA', self._get_health_policy_number())

        # Valores monetarios (solo si no es SS-Recaudo)
        if fe_operation_type != 'SS-Recaudo':
            copay = getattr(move, 'copay_amount', 0) or 0
            moderator = getattr(move, 'moderator_fee_amount', 0) or 0
            shared = getattr(move, 'shared_payment_amount', 0) or 0
            recovery = getattr(move, 'recovery_fee_amount', 0) or 0

            self._add_health_field(collection, 'COPAGO', f"{copay:.2f}")
            self._add_health_field(collection, 'CUOTA_MODERADORA', f"{moderator:.2f}")
            self._add_health_field(collection, 'PAGOS_COMPARTIDOS', f"{shared:.2f}")
            self._add_health_field(collection, 'ANTICIPO', f"{recovery:.2f}")

            # Fechas del periodo
            date_start = getattr(move, 'date_start', None) or move.invoice_date
            date_end = getattr(move, 'date_end', None) or move.invoice_date

            self._add_health_field(collection, 'FECHA_INICIO_PERIODO_FACTURACION', str(date_start) if date_start else '')
            self._add_health_field(collection, 'FECHA_FINAL_PERIODO_FACTURACION', str(date_end) if date_end else '')

        # Interoperabilidad PT (si existe)
        interop_pt = getattr(move, 'interoperability_pt_id', None)
        if interop_pt:
            self._add_interoperability_pt(interop, interop_pt)

    def _add_health_field(self, parent, name, value, scheme_id=None, scheme_name=None):
        """Agrega un campo AdditionalInformation al XML de salud"""
        add_info = ET.SubElement(parent, 'AdditionalInformation')
        name_el = ET.SubElement(add_info, 'Name')
        name_el.text = name
        value_el = ET.SubElement(add_info, 'Value')
        if scheme_id:
            value_el.set('schemeID', scheme_id)
        if scheme_name:
            value_el.set('schemeName', scheme_name)
        value_el.text = str(value) if value else ''

    def _get_health_provider_code(self):
        """Obtiene el codigo del prestador de servicios de salud"""
        move = self.document_id
        provider_code = getattr(move, 'health_provider_code', None)
        if provider_code:
            return str(provider_code).zfill(10)[:12]
        # Fallback al ref de la compania
        company_ref = move.company_id.partner_id.ref or ''
        return str(company_ref).zfill(10)[:12]

    def _get_health_payment_mode(self):
        """Obtiene la modalidad de pago"""
        move = self.document_id
        payment_mode = getattr(move, 'health_payment_mode_id', None)
        if payment_mode:
            return payment_mode.code or ''
        return ''

    def _get_health_coverage_plan(self):
        """Obtiene el plan de cobertura"""
        move = self.document_id
        coverage = getattr(move, 'health_coverage_plan_id', None)
        if coverage:
            return coverage.code or ''
        return ''

    def _get_health_contract_number(self):
        """Obtiene el numero de contrato"""
        move = self.document_id
        contract = getattr(move, 'contract_id', None)
        if contract:
            return contract.name or ''
        return ''

    def _get_health_policy_number(self):
        """Obtiene el numero de poliza"""
        move = self.document_id
        contract = getattr(move, 'contract_id', None)
        if contract and hasattr(contract, 'policy_ids') and contract.policy_ids:
            return contract.policy_ids[0].name or ''
        return ''

    def _add_interoperability_pt(self, interop, interop_pt):
        """Agrega la seccion de interoperabilidad PT"""
        interop_pt_el = ET.SubElement(interop, 'InteroperabilidadPT')

        # URL de descarga
        if interop_pt.url_download:
            url_descarga = ET.SubElement(interop_pt_el, 'URLDescargaAdjuntos')
            url_el = ET.SubElement(url_descarga, 'URL')
            url_el.text = interop_pt.url_download

            # Parametros de descarga
            if hasattr(interop_pt, 'param_excel_file') or hasattr(interop_pt, 'param_text_file'):
                params = ET.SubElement(url_descarga, 'ParametrosArgumentos')
                if getattr(interop_pt, 'param_excel_file', None):
                    param = ET.SubElement(params, 'ParametroArgumento')
                    ET.SubElement(param, 'Name').text = 'excelFile'
                    ET.SubElement(param, 'Value').text = interop_pt.param_excel_file
                if getattr(interop_pt, 'param_text_file', None):
                    param = ET.SubElement(params, 'ParametroArgumento')
                    ET.SubElement(param, 'Name').text = 'txtFile'
                    ET.SubElement(param, 'Value').text = interop_pt.param_text_file

        # Web Service
        if interop_pt.web_service_url:
            entrega = ET.SubElement(interop_pt_el, 'EntregaDocumento')
            ws_el = ET.SubElement(entrega, 'WS')
            ws_el.text = interop_pt.web_service_url
