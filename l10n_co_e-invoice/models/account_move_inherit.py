...
class AccountMoveInherit(models.Model):
    ...
    dian_invoice_period_start_date = fields.Date(string="Inicio Periodo")
    dian_invoice_period_end_date = fields.Date(string="Fin de Periodo")
    ...
    # DIAN Resolution fields - computed from journal sequence's active resolution
    resolution_number = fields.Char(
        string="Número de Resolución",
        compute="_compute_resolution_fields",
    )
    ...
    @api.depends('journal_id', 'journal_id.sequence_id', 'journal_id.sequence_id.dian_resolution_ids')
    def _compute_resolution_fields(self):
        """Compute DIAN resolution fields from the journal's active resolution"""
        for move in self:
            resolution = None
            
            # Get active DIAN resolution from journal sequence
            if move.journal_id and move.journal_id.sequence_id:
                resolution = move.journal_id.sequence_id.dian_resolution_ids.filtered(
                    lambda r: r.active_resolution
                )
                if resolution:
                    resolution = resolution[0]  # Get first (should be only one)
            
            # Set fields from resolution or False
            move.resolution_number = resolution.resolution_number if resolution else False
            move.resolution_date = resolution.date_from if resolution else False
            move.resolution_date_to = resolution.date_to if resolution else False
            move.resolution_number_from = resolution.number_from if resolution else False
            move.resolution_number_to = resolution.number_to if resolution else False
    ...
    def action_post(self):
        ...
        for record in self:
            if record.journal_id.sequence_id.use_dian_control:
                ...
                if errors:
                    raise UserError("\n".join(errors))
                if not record.journal_id.validate:
