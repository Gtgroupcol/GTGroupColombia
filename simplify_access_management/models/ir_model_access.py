...
class ir_model_access(models.Model):
    ...
    def check(self, model, mode='read', raise_exception=True):
        ...
        if model not in self.env:
            # Some access-management presets may reference models from optional
            # modules that are not installed in the current database.
            # Keep this at debug level to avoid noisy error logs on every check.
            _logger.debug('Missing model %s', model)
