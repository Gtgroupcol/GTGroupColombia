...
class costo_analytic_partner_report(models.Model):
    ...
    #                         LEFT JOIN account_analytic_plan b on a.plan_id = b.id
    #                         LEFT JOIN account_analytic_plan c on b.parent_id = c.id
    #                         LEFT JOIN account_analytic_plan d on c.parent_id = d.id
    #                         LEFT JOIN account_analytic_plan e on d.parent_id = e.id
    #                         ) as A
    #                     ) D on B.analytic_distribution ->>'account_id' = D.id::TEXT            
    #         INNER JOIN res_company G on B.company_id = G.id
    #         LEFT JOIN (
    #                     SELECT partner_id,analytic_distribution,
    #                             SUM(debit - credit) as saldo_ant 
    #                     FROM account_move_line 
    #                     WHERE "date" < '%s'  and parent_state = 'posted' 
	# 					group by partner_id,analytic_distribution
    #                   ) as E on COALESCE(B.partner_id,0) = COALESCE(E.partner_id,0) and D.id::TEXT = E.analytic_distribution ->>'account_id'
    #     ''' % (date_filter,)
