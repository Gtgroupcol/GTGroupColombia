...
class LavishReportCXCCXP(models.TransientModel):
    ...
    accounting_account_ids = fields.Many2many('account.account', string='Cuenta')
    ...
    def generate_excel(self):
        query_where = ''
        # Filtro de compañia
        query_where = query_where + f"where c.id = {self.env.company.id} and f.reconcile = True and f.account_type NOT IN ('asset_cash','liability_credit_card') "
        #Filtro fechas
        if self.cutoff_date:
        ...
        query_report = f'''
                        ...
                        from (
                            ...
                            %s
                            group by b.invoice_partner_display_name,a."date",rc."name",d."name",a.move_name,e.document_type,e.vat,b.supplier_invoice_number,f.code,f."name",
                                    b.amount_untaxed_signed, b.amount_total, b.amount_residual_signed
                        ) as a
                        group by partner,periodo,fecha_movimiento,diario,secuencia,currency,tipo_documento,ruc,td,no_factura_provedor,fecha_doc,cuenta_contable
                        order by partner,secuencia,fecha_movimiento
        ...
        for query in result_query_acumulado:
            partner = query.get('partner')  # Assuming 'Partner' is the correct key

            # Check for a change in the 'Partner' value
            if partner != partner_actual:
                if partner_actual is not None:
                    # Write subtotal rows for the previous partner
                    sheet_acumulado.write(aument_rows, columns.index('Debito'), f'=SUBTOTAL(9, R[-{group_size}]C:R[-1]C)', number_format)
                    sheet_acumulado.write(aument_rows, columns.index('Credito'), f'=SUBTOTAL(9, R[-{group_size}]C:R[-1]C)', number_format)
                    aument_rows += 1

                # Update the current partner and reset the group size
                partner_actual = partner
                group_size = 0

            # Write data to the sheet
            for col_index, key in enumerate(columns):
                value = query.get(key, '')  # Use get() to handle missing keys
                # Format date if needed
                if isinstance(value, date):
                    sheet_acumulado.write_datetime(aument_rows, col_index, value, date_format)
                else:
                    sheet_acumulado.write(aument_rows, col_index, value)

            aument_rows += 1
            group_size += 1

        # Write subtotals for the last group
        if partner_actual is not None:
            sheet_acumulado.write(aument_rows, columns.index('Debito'), f'=SUBTOTAL(9, R[-{group_size}]C:R[-1]C)', number_format)
            sheet_acumulado.write(aument_rows, columns.index('Credito'), f'=SUBTOTAL(9, R[-{group_size}]C:R[-1]C)', number_format)
        ...
        for query in result_query:
            for row in query.values():
                ...
                if str(type(row)).find('date') > -1:
                    ...
                    if isinstance(row, str) or isinstance(row, float):
                        data = row
                    elif not row:
                        data = ''
                    else:
                        data = row.get(self.env.lang)

                    sheet.write(aument_rows, aument_columns, data)
