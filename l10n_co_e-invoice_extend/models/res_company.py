...
class ResCompany(models.Model):
    ...
    def button_extract_certificate(self):
        try:
            ...
            if not password:
                raise ValueError("Contraseña del certificado no configurada")
            
            archivo_key = base64.b64decode(self.certificate_file)
            if not archivo_key:
                raise ValueError("Archivo del certificado no cargado")
            
            # Intentar cargar el certificado con la contraseña
            try:
                p12 = crypto.load_pkcs12(archivo_key, password.encode() if isinstance(password, str) else password)
            except Exception as e:
                # Si falla, intentar sin contraseña
                try:
                    p12 = crypto.load_pkcs12(archivo_key, None)
                except Exception as e2:
                    raise ValueError(f"No se puede cargar el certificado PKCS12. Verifique que:\n1. El archivo sea .p12 o .pfx\n2. La contraseña sea correcta\n3. El archivo no esté corrupto\n\nError: {str(e)}")
            
            x509 = p12.get_certificate().to_cryptography()
        except Exception as e:
            raise ValueError(_("Error al extraer certificado: %s") % str(e))
